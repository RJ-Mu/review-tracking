...
export function renderDataScreen(container, { showMessage, onBack, onDataChanged }) {
    ...
    <div class="data-screen">
      ...
      <p class="prompt" style="margin-top:14px;">// Export saves everything to a JSON file. Import validates before loading.</p>
      <div class="data-reset">
        <button class="term-btn danger" id="reset">Reset (wipe all data)</button>
        <p class="prompt" style="margin-top:8px;">// Reset cannot be undone.</p>
      </div>
    </div>
  `;
    ...
    container.querySelector('#reset').addEventListener('click', async() => {
        const ok = await confirmDialog('Wipe ALL categories and entries? This cannot be undone.', { confirmLabel: 'Wipe Everything' });
        if (!ok) return;
        try {
            await resetAll();
